...
static RevMobBanner *revMobBasicBanner = nil;

void RevMobUnityiOSBinding_releaseBasicBanner(){
    if (revMobBasicBanner != nil) {
        [revMobBasicBanner releaseAd];
        [revMobBasicBanner release];
        revMobBasicBanner = nil;
    }
}

void RevMobUnityiOSBinding_hideBasicBanner(){
    [revMobBasicBanner hideAd];
}


void RevMobUnityiOSBinding_showBasicBanner(int *orientations){
    if(revMobBasicBanner == nil){
        revMobBasicBanner = [[revmob banner] retain];
        revMobBasicBanner.supportedInterfaceOrientations = supportedInterfaceOrientations(orientations);
        revMobBasicBanner.delegate = [revmobDelegates() valueForKey:@"Banner"];
    }
    [revMobBasicBanner showAd];
}

void RevMobUnityiOSBinding_releaseBannerAd() {
    if (revmobBanner != nil) {
        [revmobBanner releaseAd];
...
void RevMobUnityiOSBinding_releaseCustomBannerAd(){
    if (revmobCustomBanner != nil) {
        [revmobCustomBanner releaseAd];
